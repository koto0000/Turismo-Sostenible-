...
public class MapaPeruApp extends JFrame {
    ...
    public MapaPeruApp() {
        ...
        setTitle("Mapa Interactivo del Perú");
        setSize(860, 400);
    ...
    private void configurarPanelMapa() {
        ...
        // MouseMotionListener para el hover en el mapa
        svgPanel.addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                if (nuevaRegion != hoveredRegionId) {
                    // Restaurar el botón anterior si existe
                    if (hoveredRegionId != null && botonesRegiones.containsKey(hoveredRegionId)) {
                        if (!hoveredRegionId.equals(selectedRegionId)) {
                            ...
                            botonesRegiones.get(hoveredRegionId).setForeground(Color.BLACK); // ← restaura texto
                        }
                    }

                    hoveredRegionId = nuevaRegion;

                    // Resaltar el nuevo botón si existe
                    if (hoveredRegionId != null && botonesRegiones.containsKey(hoveredRegionId)) {
                        if (!hoveredRegionId.equals(selectedRegionId)) {
                            botonesRegiones.get(hoveredRegionId).setBackground(new Color(220, 230, 240));
                            botonesRegiones.get(hoveredRegionId).setForeground(Color.ORANGE); // ← cambia texto
                        }
                    }
        ...
        // MouseListener para cuando el mouse sale del mapa
        svgPanel.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseExited(MouseEvent e) {
                // Restaurar el botón cuando el mouse sale del mapa
                if (hoveredRegionId != null && botonesRegiones.containsKey(hoveredRegionId)) {
                    if (!hoveredRegionId.equals(selectedRegionId)) {
                        botonesRegiones.get(hoveredRegionId).setBackground(new Color(240, 240, 240));
                    }
    ...
    private void configurarBotonesRegiones() {
        ...
        panelBotones.setLayout(new GridLayout(5, 5, 5, 5));
        ...
        panelContenedor.setBounds(260, 100, 650, 200);
    ...
    private JButton crearBotonRegion(String nombre, String id) {
        ...
        btn.setOpaque(false); 
        btn.setBorderPainted(false); // <--- Quita el borde visible
        btn.setFocusPainted(false);  // <--- Ya estaba bien
        btn.setContentAreaFilled(true); // O false si no quieres color de fondo
        btn.setBorder(null);
        ...
        btn.addMouseListener(new MouseAdapter() {
            ...
            public void mouseEntered(MouseEvent e) {
                btn.setForeground(Color.ORANGE); // Color al pasar el mouse
                hoveredRegionId = id;
                svgPanel.repaint(); // Si estás haciendo highlight en el mapa
            }

            @Override
            public void mouseExited(MouseEvent e) {
                // Restaurar color del texto al salir
                btn.setForeground(Color.BLACK);

                // Restaurar fondo solo si no está seleccionada
    ...
    private void seleccionarRegionDesdeBoton(String regionId) {
        ...
        mostrarInformacionRegion(obtenerNombreRegion(selectedRegionId));
    }
    ...
    private void seleccionarRegionDesdeMapa(String regionId) {
        ...
        // Scroll automático al botón correspondiente
    ...
    private void seleccionarRegion(String regionId) {
        ...
        btnAnterior.setForeground(Color.BLACK); // <- Añadir esto
        }
    ...
    private String obtenerNombreRegion(String idRegion) {
        for (Map.Entry<String, JButton> entry : botonesRegiones.entrySet()) {
            if (entry.getKey().equals(idRegion)) {
    ...
    private void mostrarInformacionRegion(String nombreRegion) {
        JDialog dialog = new JDialog(this, "Información", true);
        dialog.setLayout(new BorderLayout());
        
        JLabel label = new JLabel("Región seleccionada: " + nombreRegion, SwingConstants.CENTER);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            new MapaPeruApp().setVisible(true);
        });
    }
}